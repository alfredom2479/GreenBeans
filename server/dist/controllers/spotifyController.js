...
const getSpotifyClientCredsAuthToken = async () => {
...
const sendSpotifyAPIRequest = async (endpoint) => {
    console.log(endpoint);
    let returnedData = null;
    let returnedStatus = 0;
    try {
        const { data, status, statusText } = await axios.get(endpoint, { headers: { "Authorization": "Bearer " + clientCredsAuthToken } });
...
const getSpotifyRecomendations = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        */
        const { data, status, statusText } = await axios.get("https://api.spotify.com/v1/tracks/11dFghVXANMlKmJXsNCbNl", { headers: { "Authorization": "Bearer " + clientCredsAuthToken } });
        ...
        try {
            const { data, status, statusText } = await axios.get("https://api.spotify.com/v1/tracks/11dFghVXANMlKmJXsNCbNl", { headers: { "Authorization": "Bearer " + newToken } });
...
const getNewSpotifyToken = asyncHandler(async (req, res) => {
    const authHeaderString = 'Basic ' + (Buffer.from(process.env.SPOTIFY_CLIENT_ID +
        ':' + process.env.SPOTIFY_CLIENT_SECRET, "utf-8").toString('base64'));
    const authData = {
        grant_type: 'client_credentials',
...
export { getSpotifyRecomendations, getNewSpotifyToken };
